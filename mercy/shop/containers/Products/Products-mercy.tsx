    ...
    allProducts(
      where: {
        OR: [
          { type: $type }
          { title_contains: $text }
          { categories_every: { title_contains: $category } }
        ]
      }
      skip: $offset
      first: $limit
...
export const Products: React.FC<ProductsProps> = ({
  ...
  const handleLoadMore = () => {
    ...
    fetchMore({
      variables: {
        offset: Number(data.allProducts.length),
        limit: fetchLimit,
      },
      updateQuery: (prev, { fetchMoreResult }) => {
        ...
        return {
          products: {
            ...
            items: [...prev.products, ...fetchMoreResult.products],
  ...
  return (
    <>
      ...
      {loadMore && data.allProducts.hasMore && (
        <ButtonWrapper>
          <Button
